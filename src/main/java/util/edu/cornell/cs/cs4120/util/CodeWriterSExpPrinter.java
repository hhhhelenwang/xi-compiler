...
/**
 * An {@linkplain SExpPrinter} implementation designed to print ASTs through a provided {@link
 * CodeWriter}.
 */
public class CodeWriterSExpPrinter implements SExpPrinter {
    ...
    /**
     * Constructs a new {@linkplain SExpPrinter} instance that prints programs using the given
     * {@code CodeWriter}.
     *
     * @param writer the {@code CodeWriter} to print to
     */
    ...
    /**
     * Constructs a new {@linkplain SExpPrinter} instance that prints programs using the given
     * writer. Output is kept to 80 columns if possible.
     *
    ...
    /**
     * Constructs a new {@linkplain SExpPrinter} instance that prints programs using the given
     * stream. Output is kept to 80 columns if possible. Deprecated: use the previous constructor
     * instead.
     *
    ...
    protected void startElement() {
        if (addSpace) {
            if (!lists.isEmpty() && lists.get(0)) writer.unifiedBreak(0);
